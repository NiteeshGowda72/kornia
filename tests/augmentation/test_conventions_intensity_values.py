...
class TestIntensityValueRangeConventions(BaseTester):
    ...
    # Issue #4576: below p=1 the base must not let rows skipped by the probability gate
    # reach transforms that reject their values or have undefined derivatives at those values.
    # At p=0.0 RandomEqualize and RandomClahe must leave even invalid skipped rows untouched,
    # and RandomGamma must preserve the identity gradient instead of producing NaNs.
    def test_wart_p_gate_skips_samples_4576(self, device, dtype):
        if device.type == "cuda":
        ...
        assert not bool(values.grad.isnan().any())
